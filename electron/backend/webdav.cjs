// WebDAV 客户端（原生 fetch 实现，零依赖）
// 协议子集：PROPFIND / GET / PUT / MKCOL / DELETE，Basic 认证，XML multistatus 解析
"use strict";
...
// 当前同步任务的取消信号（remotesync 每轮 run 注入）：取消同步时立即中断在途
...
async function request(method, url, cfg, body, headers = {}) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      // 取消触发的 abort 不重试，直接上抛（remotesync 据此走 cancelled 收尾）
...
async function list(url, cfg) {
  ...
  if (res.status === 207 || res.status === 200) {
    ...
    const result = [];
    const seen = new Set();
    const selfHref = decodeURIComponent(new URL(url).pathname).replace(/\/+$/, "");
    for (const e of entries) {
      if (seen.has(e.href)) continue;
      seen.add(e.href);
      const decoded = decodeURIComponent(e.href);
      // 排除目录自身这条（PROPFIND Depth 1 会带自己）
      if (decoded.replace(/\/+$/, "") === selfHref) continue;
      result.push({
        href: e.href,
...
async function put(url, cfg, body) {
  const res = await request("PUT", url, cfg, body);
