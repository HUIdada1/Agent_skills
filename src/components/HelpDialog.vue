    ...
    <div v-if="app.helpOpen" class="help-overlay" @click.self="app.helpOpen = false">
      <div class="help-panel" role="dialog" aria-label="使用帮助">
        <div class="help-head">
          ...
          <div>
            <div class="strong">使用帮助</div>
            <div class="muted small">Agent_skills 是怎么工作的</div>
          </div>
          <button class="btn btn-sm help-close" @click="app.helpOpen = false"><i class="ph ph-x"></i>关闭</button>
        </div>
        ...
        <div class="help-body" ref="scroller">
          ...
          <section :id="'help-why'">
            ...
            <ul>
              ...
              <li><b>同一技能在不同工具下目录名可能不同</b>（如 <span class="mono">gpt-taste</span> / <span class="mono">taste-skill</span>），现阶段按两个技能收纳，交给去重页的 L2 归一提示处理。</li>
              <li><b>工具自带目录</b>（如 Codex 的 <span class="mono">.system</span>，内含 skill-creator、imagegen 等官方技能）由工具自己维护：<b>默认不在技能库显示，只有搜索时才会出现</b>（带「系统自带」标识），并且永不参与收纳与同步。</li>
              <li>ZCode 插件自带的技能存放在插件缓存（<span class="mono">~\.zcode\cli\plugins\cache\…</span>），不在扫描范围内。</li>
            </ul>
          </section>
          ...
          <section :id="'help-orphans'">
            ...
            <p class="muted small">开源多设备提示：每台电脑装的东西不同，孤儿列表因机器而异是正常现象；配好 WebDAV 后，跨设备以中央仓库为准。</p>
          </section>

          <section :id="'help-origin'">
            <h3><i class="ph ph-seal-check"></i>来源标识怎么看</h3>
            <ul>
              <li><span class="badge warn">系统自带</span>　工具自动维护的官方技能（目录内有 <span class="mono">.codex-system-skills.marker</span> 等标记）。<b>默认隐藏，搜索时可见</b>；软件不收纳、不挂载、不删除。</li>
              <li><span class="badge info">你安装的</span>　工具技能目录里的真实目录，来自你手动安装或技能包带入。</li>
              <li><span class="badge ok">已挂载</span>　指向中央仓库的 Junction 链接。真身只有 <span class="mono">~\.agent_skills\skills</span> 里那一份，改一处全工具生效。</li>
            </ul>
          </section>
          ...
          <section :id="'help-reference'" class="help-hl">
            ...
            <ol>
              ...
              <li>各工具默认技能目录里出现指向 <span class="mono">~\.agent_skills\skills</span> 的 Junction（复制模式则是放回一份副本）；</li>
          ...
          <section :id="'help-safety'">
            ...
            <p>默认干跑预览，你确认了才动文件；删除/覆盖先进 <span class="mono">.trash</span> 保留 7 天；同名不同内容的冲突永远人工裁决；陌生目录只标记不删除。</p>
          </section>
        </div>
      </div>
    </div>
